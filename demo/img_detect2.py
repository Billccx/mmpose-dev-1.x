...
def main():
    ...
    thr=0.3
    img=cv2.imread('demo/video/4.jpg')
    ...
    drawBBoxes(img, human_bboxes)
    drawBBoxes(img, hand_bboxes)

    for hand in hands:
        drawSkeleton(img, hand['kps'], handSkeleton)
    for human in humans:
        drawSkeleton(img, human['kps'], bodySkeleton)
